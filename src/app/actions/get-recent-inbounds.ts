...
function mapToInbound(data: any, storeName: string): Inbound {
    ...
    return {
        ...
        'Customer Name': '', // Not available in Inbounds
        'Email': '',
...
export const getRecentInboundsAction = action({
    name: 'getRecentInbounds',
    inputSchema: z.void(),
    ...
    handler: async () => {
        ...
        const allRecords: Inbound[] = [];

        for (const store of stores) {
            const url = `${WAREHOUSE_API_BASE_URL}/inbounds/?startDate=${startDate}&endDate=${endDate}&pageSize=10&page=1&col=4&colOrder=desc`;
            const data = await fetchFromParcelNinja(url, store);
            if (data?.inbounds?.length > 0) {
                const records = data.inbounds.map((item: any) => mapToInbound(item, store.name));
